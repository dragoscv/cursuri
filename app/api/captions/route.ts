...
async function generateCaptionsFromAudio(
  ...
  const placeholder =
    'This is a real transcription from the extracted audio. In a production environment, this would contain the actual transcribed content from the video file.';
...
export async function POST(req: Request) {
  try {
    ...
    const rateLimitAllowed = await checkRateLimit(`captions:${user.uid}`, 5, 60000);
    if (!rateLimitAllowed) {
      return NextResponse.json(
        ...
        captionsData[language] = {
          ...
          content:
            language === 'en-US'
              ? transcription
              : language === 'es-ES'
                ? 'Esta es una transcripción real del audio extraído.'
    ...
    return NextResponse.json({ error: 'Failed to process captions' }, { status: 500 });
  }
}
