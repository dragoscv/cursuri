...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitAllowed = await checkRateLimit(`invoice:${authenticatedUser.uid}`, 10, 60000);
    if (!rateLimitAllowed) {
      return NextResponse.json(
    ...
    const userData = userSnap.exists()
      ? userSnap.data()
      : { displayName: 'Customer', email: 'customer@example.com' };
    ...
    await logAdminAction(
      ...
      'invoice',
      paymentId,
      { userId, courseName, amount: paymentData.amount_total, currency: paymentData.currency },
      true
    );
