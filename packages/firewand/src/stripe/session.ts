...
class FirestoreSessionDAO implements SessionDAO {
    ...
    return new Promise<Session>((resolve, reject) => {
      ...
      cancel = onSnapshot(
        ...
        (snap: DocumentSnapshot<PartialSession>) => {
          ...
          // Check for errors from Stripe Extension
          if (session && (session as any).error) {
            ...
            if (errorMessage.includes('No such customer')) {
              // Extract customer ID from error if available
              const customerIdMatch = errorMessage.match(/customer: '([^']+)'/);
              const customerId = customerIdMatch ? customerIdMatch[1] : null;

              // Clear the invalid customer ID from Firestore
              this.clearInvalidCustomerId(snap.ref.parent.parent!.id, customerId).then(() => {
                reject(
                  new StripePaymentsError(
                    "not-found",
                    `Invalid Stripe customer ID was detected and cleared. Please try purchasing again. Original error: ${errorMessage}`,
                    error
                  )
                );
              }).catch((clearError) => {
                ...
                reject(
                  new StripePaymentsError(
                    "internal",
                    `Checkout failed with invalid customer: ${errorMessage}. Failed to clear customer ID automatically. Please contact support.`,
                    error
                  )
                );
              });
  ...
  private async clearInvalidCustomerId(uid: string, customerId: string | null): Promise<void> {
    try {
      ...
      console.error("Error clearing invalid customer ID:", error);
      throw error;
    }
  }
}
