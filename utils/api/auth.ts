 ...
 *
 * RATE LIMITING: Updated to use Upstash Redis for distributed, production-grade rate limiting
 *
 ...
 * Falls back to in-memory if Redis not configured (development mode)
 */
const redis =
...
/**
 * Rate limiting data structure (in-memory fallback for development)
 * In production, Upstash Redis is used for distributed caching
 */
const rateLimitStore = new Map<string, { count: number; resetTime: number }>();

/**
 * Simple rate limiting function with Upstash Redis backend
 * Falls back to in-memory for development when Redis not configured
 *
...
export async function checkRateLimit(
  ...
  if (redis) {
    try {
      ...
      console.error('Rate limiting error (Upstash):', error);
...
/**
 * Clean up expired rate limit records (only used for in-memory fallback)
 */
...
// Clean up rate limit store every 5 minutes (only for in-memory fallback)
