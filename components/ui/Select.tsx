...
const Select = forwardRef<HTMLSelectElement, SelectProps>((props, ref) => {
  ...
  const getColorStyles = () => {
    if (isInvalid)
      return 'border-[color:var(--ai-danger)] focus-within:border-[color:var(--ai-danger)] focus-within:ring-red-500/20';
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const selectedIndex = selectItems.findIndex(
        (item) => item.itemKey === (selectedKeys[0] || selectedValue)
      );
          ...
          {selectItems.length ? (
            selectItems.map((item, index) => {
              ...
              return (
                ...
                >
                  {item.startContent && (
                    <span className="mr-2 flex-shrink-0">{item.startContent}</span>
                  )}
                  ...
                  {item.endContent && (
                    <span className="ml-auto flex-shrink-0">{item.endContent}</span>
                  )}
                  {isItemSelected && (
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className="h-4 w-4 ml-2 text-[color:var(--ai-primary)]"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
                      <path
            ...
            <li
              role="presentation"
              className={`${sizeStyles.itemPadding} ${sizeStyles.text} text-[color:var(--ai-muted)] text-center`}
            >
    ...
    <div className={`relative w-full ${className}`}>
      ...
      <div ref={selectRef} className="relative w-full">
        ...
        <select
          ...
          className="sr-only"
          {...(onChange ? { value: selectedValue, onChange } : { defaultValue: selectedValue })}
          required={isRequired}
          disabled={isDisabled}
          aria-hidden="true"
          {...rest}
        ...
        >
          <div className="flex items-center flex-grow overflow-hidden">
            {startContent && <div className="flex-shrink-0 ml-2 mr-2">{startContent}</div>}
            <span
              className={`block truncate ${sizeStyles.text} ${sizeStyles.padding} ${
                selectedValue ? 'text-[color:var(--ai-foreground)]' : 'text-[color:var(--ai-muted)]'
              }`}
            >
