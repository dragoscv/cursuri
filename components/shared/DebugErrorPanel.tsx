...
function readNetworkInfo(): string | null {
  ...
  if (typeof conn.rtt === 'number') parts.push(`rtt=${conn.rtt}ms`);
...
function buildReport(
  ...
  return {
    ...
    online: isBrowser && typeof navigator.onLine === 'boolean' ? navigator.onLine : null,
    network: isBrowser ? readNetworkInfo() : null,
  };
}
...
function reportToMarkdown(r: DebugReport): string {
  return [
    ...
    r.online !== null ? `- **Online:** \`${r.online}\`` : null,
    r.network ? `- **Network:** \`${r.network}\`` : null,
    ...
    `${r.name}: ${r.message}`,
    '```',
    r.stack ? ['', '### Stack', '', '```', r.stack, '```'].join('\n') : null,
    r.cause ? ['', '### Cause', '', '```', r.cause, '```'].join('\n') : null,
  ]
    .filter(Boolean)
    .join('\n');
}
      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-[color:var(--ai-muted)]">
              An unexpected error stopped this page from rendering. Copy the report below and share
              it with the team — it has everything needed to reproduce and fix.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-[color:var(--ai-card-border)] bg-[color:var(--ai-card-bg)]/60 backdrop-blur-sm overflow-hidden">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 px-5 py-4 border-t border-[color:var(--ai-card-border)] text-xs">
            ...
            {report.sentryEventId && <Row label="Sentry event" value={report.sentryEventId} mono />}
            {report.appVersion && <Row label="App version" value={report.appVersion} mono />}
            {report.online !== null && (
              <Row label="Online" value={report.online ? 'yes' : 'no'} mono />
            )}
            {report.network && <Row label="Network" value={report.network} mono />}
