...
interface AuthContextType {
    ...
    saveUserPreferences: (preferences: Partial<UserPreferences>) => Promise<{ success: boolean; error?: string }>;
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    const createUserProfile = useCallback(async (user: User, displayName?: string): Promise<UserProfile> => {
        const userProfile: UserProfile = {
            id: user.uid,
            email: user.email || '',
            displayName: displayName || user.displayName || '',
            photoURL: user.photoURL || undefined, role: 'user' as UserRole,
            isActive: true,
            permissions: {
    ...
    useEffect(() => {
        ...
        return () => unsubscribe();
    }, [checkAdminStatus, loadUserProfile, loadUserPreferences, createUserProfile, createUserPreferences]);
