...
export const AdminProvider: React.FC<AdminProviderProps> = ({ children }) => {
  ...
  const getUsers = useCallback(
    async (pageSize = 20, refresh = false): Promise<PaginatedResult<UserProfile>> => {
      if (!user || !isAdmin) {
      ...
      try {
        let q;
        const lastDoc = refresh ? null : state.lastDocs['users'];
  ...
  const updateUser = useCallback(
    async (uid: string, data: Partial<UserProfile>): Promise<boolean> => {
      if (!user || !isAdmin) {
        console.error('Unauthorized access to admin function');
        return false;
      }
  ...
  const getAdminStats = useCallback(async (): Promise<AdminStats | null> => {
    ...
    try {
      ...
      const paymentsQuery = query(collection(db, 'payments'), where('status', '==', 'succeeded'));
  ...
  const getPendingReviews = useCallback(
    async (pageSize = 20, refresh = false) => {
      if (!user || !isAdmin) {
        console.error('Unauthorized access to admin function');
        return { items: [], lastDoc: null, hasMore: false };
      }
      ...
      try {
        let q;
        const lastDoc = refresh ? null : state.lastDocs['pendingReviews'];
  ...
  const uploadCourseImage = useCallback(
    async (courseId: string, file: File): Promise<string> => {
      if (!user || !isAdmin) {
        console.error('Unauthorized access to admin function');
