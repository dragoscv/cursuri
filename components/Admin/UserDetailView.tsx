...
import { Avatar, Button, Chip, Spinner, Input } from '@heroui/react';
...
interface AdminUserData {
  ...
  enrollments: Record<
    string,
    { enrolledAt?: { seconds: number } | string; source?: string; status?: string }
  >;
}
...
export default function UserDetailView({ userId }: { userId: string }) {
  ...
  // Sync tab to URL via history.replaceState — avoids router.push triggering
  // a [userId] segment re-render that races with local state under React 19.
  ...
  // Pick up tab changes that come from outside (back/forward buttons,
  ...
  if (error || !user) {
    return (
      <div className="text-center py-16">
        ...
        <h2 className="text-xl font-bold text-[color:var(--ai-foreground)] mb-2">
    ...
    <div className="space-y-6">
      ...
      <div>
          ...
          >
            {(
              [
                ...
                { key: 'enrollments', label: `📚 Enrollments (${enrollmentCount})` },
                { key: 'github', label: '🐙 GitHub Accounts' },
                { key: 'subscriptions', label: `💎 Subscriptions (${subscriptions.length})` },
                { key: 'activity', label: '📈 Activity' },
                { key: 'notes', label: '📝 Admin Notes' },
                { key: 'permissions', label: '🛡️ Permissions' },
              ...
              return (
                <button
                  ...
                  className={`relative py-3 px-2 sm:px-3 font-medium whitespace-nowrap transition-colors ${
                    isActive
                      ? 'text-[color:var(--ai-foreground)]'
                      : 'text-[color:var(--ai-muted-foreground)] hover:text-[color:var(--ai-foreground)]'
                  }`}
                >
                  {tab.label}
                  {isActive && (
                    <span
                      aria-hidden
                      className="absolute -bottom-px left-0 right-0 h-0.5 bg-[color:var(--ai-primary)]"
                    />
                  )}
                </button>
              );
        ...
        <div className="pt-6">
                ...
                <div className="space-y-4 max-w-2xl">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectItem key="super_admin" value="super_admin" textValue="super_admin">
                        Super Admin
                      </SelectItem>
                    </Select>
                  </div>
          ...
          {/* Activity Tab */}
          {selectedTab === 'activity' && <UserActivityTimeline userId={userId} />}
              ...
              <div className="bg-[color:var(--ai-card-bg)]/60 rounded-xl p-4 border border-[color:var(--ai-card-border)]">
                ...
                <div className="space-y-2">
                  ...
                  <PermissionRow
                    ...
                    enabled={isAdminRole || user.role === 'instructor'}
                  />
                </div>
              </div>
