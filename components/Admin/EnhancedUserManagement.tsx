...
import { UserProfile, AppContextProps } from '@/types';
...
const EnhancedUserManagement: React.FC = () => {
  ...
  const tUser = useTranslations('admin.userManagement');
  const tError = useTranslations('admin.users.errors');
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        setError(tUser('failedToLoadUserData'));
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <h1 className="text-3xl font-bold">{tUser('title')}</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              >
                ...
                <SelectItem key="instructor" value="instructor" textValue="instructor">
                  Instructors
                </SelectItem>
              </Select>
            </div>
            ...
            <div>
              ...
              >
                ...
                <SelectItem key="verified" value="verified" textValue="verified">
                  {tUser('verified')}
                </SelectItem>
                <SelectItem key="unverified" value="unverified" textValue="unverified">
                  {tUser('unverified')}
                </SelectItem>
              </Select>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <Select
                  ...
                  className="flex-1"
                >
                  <SelectItem key="createdAt" value="createdAt" textValue="createdAt">
                    {tUser('registrationDate')}
                  </SelectItem>
                  <SelectItem key="displayName" value="displayName" textValue="displayName">
                    {tUser('displayName')}
                  </SelectItem>
                </Select>
                <Button
      ...
      <Card className="shadow-md">
        <CardBody>
          <Table aria-label="Enhanced users table">
            <TableHeader>
              <TableColumn>{tCommon('user').toUpperCase()}</TableColumn>
              <TableColumn>{tUser('emailHeader')}</TableColumn>
              <TableColumn>{tCommon('role').toUpperCase()}</TableColumn>
              <TableColumn>{tUser('verification')}</TableColumn>
              <TableColumn>{tUser('enrollment')}</TableColumn>
              <TableColumn>{tUser('registration')}</TableColumn>
              <TableColumn>{tUser('actions')}</TableColumn>
            </TableHeader>
            <TableBody>
              {paginatedUsers.length > 0 ? (
                paginatedUsers.map((user) => (
                  <TableRow
                    key={user.id}
                    className="cursor-pointer hover:bg-[color:var(--ai-card-bg)] dark:hover:bg-[color:var(--ai-card-border)]"
                  ...
                  <ModalBody className="py-6">
                        ...
                        {editMode ? (
                          ...
                          <>
                            <div className="bg-[color:var(--ai-card-bg)]/80 dark:bg-[color:var(--ai-card-border)]/50 p-4 rounded-xl backdrop-blur-sm space-y-4">
                              <div>
                                <h3 className="text-sm font-medium text-[color:var(--ai-muted-foreground)] dark:text-[color:var(--ai-muted-foreground)]">
                                  Display Name
                                </h3>
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-2">
                                      <Chip
                                        color={
                                          enrollment.source === 'admin' ? 'primary' : 'success'
                                        }
                                        size="sm"
                                      >
                    ...
                    {selectedTab === 'notes' && (
                      <div className="space-y-4">
                        ...
                        <p className="text-sm text-[color:var(--ai-muted-foreground)] dark:text-[color:var(--ai-muted-foreground)]">
                          These notes are only visible to administrators and are not shown to the
                          user.
                        </p>
                                ...
                                <div key={courseId} className="relative">
                                  ...
                                  <div className="flex flex-col">
                                    <p className="font-medium">
                                      Enrolled in {courses[courseId]?.name || 'a course'}
                                    </p>
                                    <p className="text-sm text-[color:var(--ai-muted-foreground)] dark:text-[color:var(--ai-muted-foreground)]">
                                      {enrollment.enrolledAt
                                        ? typeof enrollment.enrolledAt === 'object' &&
                                    ...
                                    <p className="text-xs text-[color:var(--ai-muted-foreground)] dark:text-[color:var(--ai-muted-foreground)] mt-1">
                                      {t('userManagement.enrollment.via')}:{' '}
                                      {enrollment.source === 'admin'
                                        ? t('userManagement.enrollment.adminAssignment')
                                        : t('userManagement.enrollment.selfPurchase')}
                                    </p>
                                  </div>
                                </div>
                              )
                          ...
                          <div className="bg-[color:var(--ai-card-bg)]/80 dark:bg-[color:var(--ai-card-border)]/50 p-4 rounded-xl backdrop-blur-sm">
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center justify-between py-2 border-b border-[color:var(--ai-card-border)] dark:border-[color:var(--ai-card-border)]">
                                ...
                                <Chip
                                  size="sm"
                                  color={selectedUser.role === 'admin' ? 'success' : 'danger'}
                                >
