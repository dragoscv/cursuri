...
export default function Course({ courseId }: CourseProps) {
  ...
  const hasAccess = userPaidProducts?.find(
    (userPaidProduct: UserPaidProduct) => userPaidProduct.metadata.courseId === courseId
  );

  useEffect(() => {
    // Home/catalog card: use one-shot reads (no persistent Firestore Listen
    // channels). Avoids leaking long-poll listeners on each course card.
    let cancelled = false;
    let unsubReviews: (() => void) | undefined;
    let unsubLessons: (() => void) | undefined;
    (async () => {
      const r = await getCourseReviews(courseId, { realtime: false });
      const l = await getCourseLessons(courseId, { realtime: false });
      if (cancelled) {
        if (typeof r === 'function') r();
        if (typeof l === 'function') l();
        return;
      }
      if (typeof r === 'function') unsubReviews = r;
      if (typeof l === 'function') unsubLessons = l;
    })();
    return () => {
      cancelled = true;
      unsubReviews?.();
  ...
  const sortedLessons = (): Lesson[] => {
    ...
    if (!lessons[courseId]) {
      console.warn(
        `No lessons found for courseId: ${courseId}. This could be because they're still loading.`,
        { lessons }
      );
      return [];
    }
  ...
  const calculateCourseProgress = () => {
    ...
    const completedLessons = lessonIds.filter((lessonId) => isLessonCompleted(lessonId)).length;
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
            ...
            <Button
              ...
              onClick={() =>
                openModal({
                  ...
                  modalBody: (
                    <AddLesson courseId={courseId} onClose={() => closeModal('addLesson')} />
                  ),
                  footerDisabled: true,
                  noReplaceURL: true,
          ...
          <div className="w-full md:w-auto min-w-[280px]">
            <CourseEnrollment course={course} isPurchased={!!hasAccess || isAdmin} />
          </div>
        </div>
      ...
      <div className="mt-4">
        ...
        {selectedTab === 'content' && (
          <CourseContent
            ...
            isAdmin={isAdmin}
            completedLessons={
              lessonProgress && lessonProgress[courseId]
                ? Object.keys(lessonProgress[courseId]).reduce(
                    (acc, lessonId) => {
            ...
            <div className="space-y-3">
              {course.resources.map((resource: Resource, index: number) => (
                <div key={index} className="flex items-center p-3 border rounded-lg">
                  <svg
                    className="w-5 h-5 mr-3 text-[color:var(--ai-primary)]"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg"
                  >
