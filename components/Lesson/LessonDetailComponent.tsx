...
export default function LessonDetailComponent({
  ...
  const { courses, lessons, user, userPaidProducts, isAdmin, getCourseLessons, subscriptions } =
    context;
  ...
  useEffect(() => {
    // Only fetch lessons if they don't exist yet. One-shot read (no
    // persistent Firestore Listen channel) — lessons don't need realtime
    // while the user is viewing a single lesson, and the prior effect
    // never cleaned up the listener.
    if (
      courseId &&
      courses &&
      courses[courseId] &&
      ...
      getCourseLessons(courseId, { realtime: false });
    }
            ...
            <div className="space-y-4">
              ...
              <div className="mt-6 p-4 bg-[color:var(--ai-card-bg)] dark:bg-[color:var(--ai-card-border)] rounded-lg text-left">
                ...
                <ul className="list-disc pl-5 space-y-1 text-sm">
                  <li>
                    {t('courseId')} {debug.courseId}
                  </li>
                  <li>
                    {t('lessonId')} {debug.lessonId}
                  </li>
                  <li>
                    {t('courseExists')} {debug.courseExists ? t('yes') : t('no')}
                  </li>
                  <li>
                    {t('courseLessonsLoaded')} {debug.courseLessonsExist ? t('yes') : t('no')}
                  </li>
                  <li>
                    {t('lessonsAvailable')} {debug.lessonsAvailable ? t('yes') : t('no')}
                  </li>
                  <li>
                    {t('lessonExists')} {debug.lessonExists ? t('yes') : t('no')}
                  </li>
                </ul>
                <div className="mt-2">
                  <button
