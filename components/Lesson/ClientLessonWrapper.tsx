...
export default function ClientLessonWrapper({ params }: ClientLessonWrapperProps) {
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen p-4">
                ...
                {context && (
                    <div className="mb-6 p-4 bg-[color:var(--ai-card-bg)] border border-[color:var(--ai-card-border)] rounded-lg text-left max-w-2xl w-full">
                        ...
                        <div className="space-y-1 text-xs font-mono">
                            ...
                            <div>
                                <strong>{t('lessonsLoaded')}</strong>{' '}
                                {context.lessons[courseId] ? t('yes') : t('no')}
                            </div>
                            {context.lessons[courseId] && (
                                <>
                                    <div>
                                        <strong>{t('availableLessonCount')}</strong>{' '}
                                        {Object.keys(context.lessons[courseId]).length}
                                    </div>
                                    <div>
                ...
                <button
                    ...
                    className="px-6 py-2 bg-gradient-to-r from-[color:var(--ai-primary)] to-[color:var(--ai-secondary)] text-[color:var(--ai-foreground)] rounded-lg hover:opacity-90 transition-all duration-300"
                >
                    {t('returnToCourse')}
                </button>
            </div>
    ...
    if (lesson && course && accessCheckComplete) {
        ...
        if (context && context.lessons[courseId]) {
            const courseLessons = Object.values(context.lessons[courseId]) as LessonType[];
            ...
            if (currentIndex !== -1) {
                if (currentIndex > 0) {
                    prevLessonId = sortedLessons[currentIndex - 1].id;
                }
