...
export default function ClientLessonWrapper({ params }: ClientLessonWrapperProps) {
  ...
  const accessReason: AccessReason = useMemo(() => {
    ...
    // Wait for auth to finish resolving before deciding access — otherwise
    // an admin can briefly see "access denied" while the auth handler is
    // still loading the user profile and isAdmin is still false.
    if (context.authLoading) return 'pending';
