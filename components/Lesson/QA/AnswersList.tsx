      ...
      {displayAnswers.map((answer) => (
        <div key={answer.id} className="border-t border-[color:var(--ai-card-border)]/20 pt-4">
          <div className="flex justify-between items-start mb-3">
            <div className="flex items-center gap-3">
              <Avatar
                ...
                size="sm"
                className={`border-2 ${answer.isAuthorOrAdmin ? 'border-amber-500/50' : 'border-[color:var(--ai-card-border)]/40'}`}
              />
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {answer.userRole && (
                    <span
                      className={`text-[10px] uppercase tracking-[0.18em] font-semibold px-2 py-0.5 rounded-md ${
                        answer.userRole === 'Instructor' || answer.userRole === 'Admin'
                          ? 'bg-amber-500/[0.1] text-amber-500'
                          : 'bg-[color:var(--ai-foreground)]/10 text-[color:var(--ai-foreground)]'
                      }`}
                    >
                      {answer.userRole}
                    </span>
          ...
          <div className="mt-2">
            {answer.htmlContent ? (
              <div
                className="prose dark:prose-invert max-w-none prose-img:rounded-lg prose-img:shadow-none prose-a:text-amber-500 text-[color:var(--ai-text-secondary)]"
