...
export default function LessonTimeline({
  ...
  const bufferPct = bufferedSeconds ? Math.min(100, (bufferedSeconds / safeDuration) * 100) : 0;
  ...
  const computeSeekFromClientX = useCallback(
    (clientX: number) => {
  ...
  const hoverChapter =
    hoverTime != null && sortedChapters.length > 0
        ...
        <div className="relative h-3 mb-1">
          {sortedChapters.map((c, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`block h-3 w-[2px] rounded-full transition-all ${isActive ? 'bg-amber-500' : 'bg-white/70 group-hover/chap:bg-white'}`}
                />
        ...
        {hasWaveform ? (
          ...
          >
            {renderedPeaks!.slice(0, PEAK_BUCKETS).map((p, i) => {
              ...
              const fill = isPlayed
                ? isSpeech
                  ? 'var(--ai-primary, #60a5fa)'
                  : 'rgba(96,165,250,0.35)'
                : isSpeech
                  ? 'rgba(255,255,255,0.85)'
                  : 'rgba(255,255,255,0.25)';
              return (
                <rect key={i} x={i + 0.15} y={50 - h / 2} width={0.7} height={h} fill={fill} />
              );
            })}
          </svg>
        ) : (
