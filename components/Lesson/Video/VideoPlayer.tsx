  ...
  useEffect(() => {
    ...
    const handleError = () => {
      ...
      const isUnsupportedFormat =
        fileName.endsWith('.webm') || fileName.endsWith('.mkv') || fileName.endsWith('.avi');

      if (isUnsupportedFormat && isSafari) {
        setVideoError(
          'This video format is not supported by your browser. Please try using Chrome or Firefox.'
        );
      } else if (error) {
        switch (error.code) {
          ...
          case MediaError.MEDIA_ERR_NETWORK:
            setVideoError(
              'A network error occurred while loading the video. Please check your connection and try again.'
            );
            break;
  ...
  return (
    <>
      <div
        ...
        className={`relative overflow-hidden rounded-2xl bg-black border border-[color:var(--ai-card-border)] group ${isFullscreen ? 'flex items-center justify-center' : ''}`}
                  ...
                  <Dropdown>
                    <DropdownTrigger>
                      <HeroButton
                        variant="light"
                        size="sm"
                        className={captionsEnabled ? 'text-white bg-amber-500/25' : 'text-white'}
                        startContent={<CaptionsIcon />}
                        aria-label={t('selectCaptions')}
                      >
                        {selectedCaptionLanguage === 'off'
                          ? 'Off'
                          : selectedCaptionLanguage.toUpperCase()}
                      </HeroButton>
                    </DropdownTrigger>
                    <DropdownMenu
                      aria-label={t('selectCaptions')}
                    ...
                    >
                      <DropdownItem key="off">{t('captionsOff') || 'Off'}</DropdownItem>
                      {
                        Object.keys(lesson.captions).map((language) => (
                          <DropdownItem key={language}>{language.toUpperCase()}</DropdownItem>
                        )) as any
                      }
                    </DropdownMenu>
                  </Dropdown>
