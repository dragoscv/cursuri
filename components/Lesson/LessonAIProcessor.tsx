...
import { FiPlay, FiCheckCircle, FiAlertCircle, FiClock } from '@/components/icons/FeatherIcons';
import LessonAIProgressModal, { type AiProgressState, type AiStage } from './LessonAIProgressModal';
...
const LessonAIProcessor: React.FC<LessonAIProcessorProps> = ({
  ...
  return (
    <div className="rounded-2xl border border-[color:var(--ai-card-border)] bg-[color:var(--ai-card-bg)] p-5 sm:p-6">
      <div className="flex flex-wrap items-start gap-3 justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-[color:var(--ai-muted)] mt-1 max-w-prose">
            ...
            generates WEBVTT captions, and summarizes it via Azure OpenAI. Jobs run on the server —
            you can close this dialog and navigate freely; the global AI Jobs tray (top bar) shows
      ...
      <LessonAIProgressModal
        ...
        onApply={onApply}
        onCancel={
          job && (job.status === 'queued' || job.status === 'processing') ? handleCancel : undefined
        }
